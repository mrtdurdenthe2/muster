...
import type { RepositoryIssuesTab } from "./issueTab.js"
...
test("builds qualified repository search queries", () => {
  expect(repositoryIssueSearchQuery("owner/repo", "name:startup author:@alice tag:type:bug")).toBe(
    'repo:owner/repo is:issue "startup" author:alice label:"type:bug" in:title sort:updated-desc',
  )
  expect(repositoryIssueSearchQuery("owner/repo", "startup @alice type:bug")).toBe(
    'repo:owner/repo is:issue "startup" author:alice label:"type:bug" in:title sort:updated-desc',
  )
})
...
test("exact number search fetches the issue directly", async () => {
  ...
  const result = await Effect.runPromise(
    searchRepositoryIssues(tab, "number:42 author:alice").pipe(Effect.provideService(GitHubIssues, service)),
  )
